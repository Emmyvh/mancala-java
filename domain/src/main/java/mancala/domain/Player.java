...
public class Player {
    ...
    // Defining the variables. Each player has their own cups, a kalaha, and
    // opponent cups.
    ...
    // Getters for score, total number of stones for this player, list Cups, and
    // List opponent Cups.
    public int getScore() {
    ...
    public int getNumberOfStonesPerPlayer() {
        int numberOfStonesInAllCups = getCups().get(0).getStonesPerCup() + getCups().get(1).getStonesPerCup()
                + getCups().get(2).getStonesPerCup() + getCups().get(3).getStonesPerCup()
                + getCups().get(4).getStonesPerCup() + getCups().get(5).getStonesPerCup();
    ...
    public StoneCollectors makeMove(int selectedCupNumber, List<Cup> opponentCups) {
        ...
        // Declare a variable as the selected cup and declare a variable with its number
        // of stones.
        ...
        // Assign an Empty variable by declaring it null. This throws an exception if
        // not used in the for loop.
        ...
        capture(lastCup, opponentCups);

        return activeElement;
    }

    private void capture(Cup lastCup, List<Cup> opponentCups) {
